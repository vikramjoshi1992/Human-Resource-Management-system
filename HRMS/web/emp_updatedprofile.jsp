...
<%
	...
	String ename=request.getParameter("empname").toLowerCase();
        String sex=request.getParameter("gender").toLowerCase();
        String epwd=request.getParameter("pwd").toLowerCase();
	String addr=request.getParameter("address").toLowerCase();
	String phone=request.getParameter("contactno");
	String aemail=request.getParameter("email").toLowerCase();
...
<%	Connection con=null;
	...
	try{
		...
		String query="update employee set empname = '"+ename+"',address = '"+addr+"' , contactno = "+Integer.parseInt(phone)+" , aemail = '"+aemail+"' , pword = '"+epwd+"' , gender = '"+sex+"'  where empid='"+eid+"'";
				...
				if(eff>0 )
					out.println("<br/><h2 align=center>'Hurrey!Your Profile Successfully Updated'<BR></h2>");
